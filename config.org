...
Package declarations live right above the configuration that uses
them, rather than in a separate =packages.el=. That file is now a
generated artifact (like =init.el= and =config.el=) and isn't tracked
in git — run =doom sync= after adding or removing a =package!= block.
