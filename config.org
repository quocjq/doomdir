...
=org-auto-tangle=. The =:tangle init.el= header on the Doom modules
block sends it to =init.el=; every other block tangles to =config.el=.
...
- =scroll-margin= keeps a couple of lines of context above/below the
...
enters the emphasized region, so editing stays easy. The package is
already declared in =packages.el=.
...
* Agenda
tecosaur's souped-up agenda, built on =org-super-agenda=. The package
is declared in =packages.el=. This groups the =alltodo= view by tag,
priority, and deadline so a single =SPC o A o= "Overview" gives you
Today's time-grid plus everything else bucketed sensibly.
...
and the =org-roam-ui= graph view. All three packages (=org-roam=,
=org-roam-ui=, =websocket=) are already declared in =packages.el=.
